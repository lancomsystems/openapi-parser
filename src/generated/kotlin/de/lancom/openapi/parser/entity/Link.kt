...
data class Link(
    ...
    fun getFields(): Set<Fields> {
        val allFields: Set<Fields> = setOfNotNull(
            setOfNotNull<Fields>(
                if (_operationId.isDefined) Fields.operationId else null,
                if (_operationRef.isDefined) Fields.operationRef else null,
                if (_parameters.isDefined) Fields.parameters else null,
                if (_requestBody.isDefined) Fields.requestBody else null,
                if (_description.isDefined) Fields.description else null,
                if (_server.isDefined) Fields.server else null,
            ),
    ...
    // hint:D6150566
    fun mergeFields(other: Link): Link {
        return copy(
    ...
    override fun getReferencePath(): String? {
        return if (__referenceName == null) {
            ...
            "#/components/links/$__referenceName"
        }
    }

    fun setReferenceName(name: String): Link {
    ...
    companion object : ReferenceParser<Link> {
        ...
        data class Fields(val value: String) {
            ...
            companion object {
                val operationId = Fields("operationId")
                val operationRef = Fields("operationRef")
                val parameters = Fields("parameters")
                val requestBody = Fields("requestBody")
