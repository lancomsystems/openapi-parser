...
data class Schema(
    ...
    fun mergeProperties(propertiesToMerge: Map<String, ReferenceOrInstance<Schema>?>?): Schema {
        return if (propertiesToMerge == null) {
            ...
            if (oldProperties == null) {
                ...
                // hint:70A3D8B6
                setProperties(de.lancom.openapi.parser.util.mergeMap(oldProperties, propertiesToMerge))
            }
        }
    }
    ...
    fun getFields(): Set<Fields> {
        val allFields: Set<Fields> = setOfNotNull(
            setOfNotNull<Fields>(
                if (_title.isDefined) Fields.title else null,
                if (_multipleOf.isDefined) Fields.multipleOf else null,
                if (_maximum.isDefined) Fields.maximum else null,
                if (_exclusiveMaximum.isDefined) Fields.exclusiveMaximum else null,
                if (_minimum.isDefined) Fields.minimum else null,
                if (_exclusiveMinimum.isDefined) Fields.exclusiveMinimum else null,
                if (_maxLength.isDefined) Fields.maxLength else null,
                if (_minLength.isDefined) Fields.minLength else null,
                if (_pattern.isDefined) Fields.pattern else null,
                if (_maxItems.isDefined) Fields.maxItems else null,
                if (_minItems.isDefined) Fields.minItems else null,
                if (_uniqueItems.isDefined) Fields.uniqueItems else null,
                if (_maxProperties.isDefined) Fields.maxProperties else null,
                if (_minProperties.isDefined) Fields.minProperties else null,
                if (_required.isDefined) Fields.required else null,
                if (_enum.isDefined) Fields.enum else null,
                if (_type.isDefined) Fields.type else null,
                if (_not.isDefined) Fields.not else null,
                if (_allOf.isDefined) Fields.allOf else null,
                if (_oneOf.isDefined) Fields.oneOf else null,
                if (_anyOf.isDefined) Fields.anyOf else null,
                if (_items.isDefined) Fields.items else null,
                if (_properties.isDefined) Fields.properties else null,
                if (_additionalProperties.isDefined) Fields.additionalProperties else null,
                if (_description.isDefined) Fields.description else null,
                if (_format.isDefined) Fields.format else null,
                if (_default.isDefined) Fields.default else null,
                if (_nullable.isDefined) Fields.nullable else null,
                if (_discriminator.isDefined) Fields.discriminator else null,
                if (_readOnly.isDefined) Fields.readOnly else null,
                if (_writeOnly.isDefined) Fields.writeOnly else null,
                ...
                if (_xml.isDefined) Fields.xml else null,
            ),
    ...
    // hint:D6150566
    fun mergeFields(other: Schema): Schema {
        return copy(
    ...
    override fun getReferencePath(): String? {
        return if (__referenceName == null) {
            ...
            "#/components/schemas/$__referenceName"
        }
    }

    fun setReferenceName(name: String): Schema {
    ...
    companion object : ReferenceParser<Schema> {
        ...
        data class Fields(val value: String) {
            ...
            companion object {
                ...
                val multipleOf = Fields("multipleOf")
                val maximum = Fields("maximum")
                val exclusiveMaximum = Fields("exclusiveMaximum")
                val minimum = Fields("minimum")
                val exclusiveMinimum = Fields("exclusiveMinimum")
                val maxLength = Fields("maxLength")
                val minLength = Fields("minLength")
                val pattern = Fields("pattern")
                val maxItems = Fields("maxItems")
                val minItems = Fields("minItems")
                val uniqueItems = Fields("uniqueItems")
                val maxProperties = Fields("maxProperties")
                val minProperties = Fields("minProperties")
                val required = Fields("required")
                val enum = Fields("enum")
                val type = Fields("type")
                val not = Fields("not")
                val allOf = Fields("allOf")
                val oneOf = Fields("oneOf")
                val anyOf = Fields("anyOf")
                val items = Fields("items")
                val properties = Fields("properties")
                val additionalProperties = Fields("additionalProperties")
                val description = Fields("description")
                val format = Fields("format")
                val default = Fields("default")
                val nullable = Fields("nullable")
                val discriminator = Fields("discriminator")
                val readOnly = Fields("readOnly")
                val writeOnly = Fields("writeOnly")
