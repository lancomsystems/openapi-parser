...
@JsonSerialize(using = RequestBodyJson.Companion.Serializer::class)
@JsonDeserialize(using = RequestBodyJson.Companion.Deserializer::class)
data class RequestBodyJson(
    ...
    // hint:F0C48D71
    fun setJsonNodeField(jsonNode: Field<JsonNode>): RequestBodyJson {
        return copy(_jsonNode = jsonNode)
            .updateFields()
    }

    // hint:8E56A4D9
    fun updateJsonNodeField(updater: (Field<JsonNode>) -> Field<JsonNode>): RequestBodyJson {
        return setJsonNodeField(updater(_jsonNode))
    }

    // hint:B1D730FC
    fun updateJsonNode(updater: (JsonNode) -> JsonNode): RequestBodyJson {
    ...
    // hint:6542E98A
    fun mergeJsonNodeField(jsonNodeFieldToMerge: Field<JsonNode>): RequestBodyJson {
        return mergeJsonNode(jsonNodeFieldToMerge.orNull)
    }

    // hint:A8BC6F23
    fun mergeJsonNode(jsonNodeToMerge: JsonNode?): RequestBodyJson {
    ...
    // hint:87B3E19C
    fun setJsonNode(jsonNode: JsonNode): RequestBodyJson {
        return setJsonNodeField(Field(jsonNode))
    }

    // hint:D465F782
    fun unsetJsonNode(): RequestBodyJson {
        return setJsonNodeField(Field.unset())
    }

    // hint:47C9A0F6
    fun addJsonNode(jsonNode: JsonNode): RequestBodyJson {
    ...
    // hint:EFD0D818
    fun updateFields(): RequestBodyJson {
        return copy(__field_order = getFields())
    }

    // hint:A0E5F382
    override fun mergeEntity(other: Entity?): RequestBodyJson {
        return when (other) {
            null ->
                this
    ...
    // hint:D6150566
    fun mergeFields(other: RequestBodyJson): RequestBodyJson {
        return copy(
    ...
    // hint:716BFD54
    fun merge(other: RequestBodyJson?): RequestBodyJson {
    ...
    companion object : JsonEntityFactory<RequestBodyJson> {
        ...
        class Serializer : EntitySerializer<RequestBodyJson>(RequestBodyJson::class.java, RequestBodyJson)
        class Deserializer : EntityDeserializer<RequestBodyJson>(RequestBodyJson::class.java, RequestBodyJson)

        // hint:5F72B6D8
        override fun parseWrapper(wrapper: Wrapper): RequestBodyJson {
