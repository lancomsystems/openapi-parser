...
@JsonSerialize(using = DefaultJson.Companion.Serializer::class)
@JsonDeserialize(using = DefaultJson.Companion.Deserializer::class)
data class DefaultJson(
    ...
    // hint:F0C48D71
    fun setJsonNodeField(jsonNode: Field<JsonNode>): DefaultJson {
        return copy(_jsonNode = jsonNode)
            .updateFields()
    }

    // hint:8E56A4D9
    fun updateJsonNodeField(updater: (Field<JsonNode>) -> Field<JsonNode>): DefaultJson {
        return setJsonNodeField(updater(_jsonNode))
    }

    // hint:B1D730FC
    fun updateJsonNode(updater: (JsonNode) -> JsonNode): DefaultJson {
    ...
    // hint:6542E98A
    fun mergeJsonNodeField(jsonNodeFieldToMerge: Field<JsonNode>): DefaultJson {
        return mergeJsonNode(jsonNodeFieldToMerge.orNull)
    }

    // hint:A8BC6F23
    fun mergeJsonNode(jsonNodeToMerge: JsonNode?): DefaultJson {
    ...
    // hint:87B3E19C
    fun setJsonNode(jsonNode: JsonNode): DefaultJson {
        return setJsonNodeField(Field(jsonNode))
    }

    // hint:D465F782
    fun unsetJsonNode(): DefaultJson {
        return setJsonNodeField(Field.unset())
    }

    // hint:47C9A0F6
    fun addJsonNode(jsonNode: JsonNode): DefaultJson {
    ...
    // hint:EFD0D818
    fun updateFields(): DefaultJson {
        return copy(__field_order = getFields())
    }

    // hint:A0E5F382
    override fun mergeEntity(other: Entity?): DefaultJson {
        return when (other) {
            null ->
                this
    ...
    // hint:D6150566
    fun mergeFields(other: DefaultJson): DefaultJson {
        return copy(
    ...
    // hint:716BFD54
    fun merge(other: DefaultJson?): DefaultJson {
    ...
    companion object : JsonEntityFactory<DefaultJson> {
        ...
        class Serializer : EntitySerializer<DefaultJson>(DefaultJson::class.java, DefaultJson)
        class Deserializer : EntityDeserializer<DefaultJson>(DefaultJson::class.java, DefaultJson)

        // hint:5F72B6D8
        override fun parseWrapper(wrapper: Wrapper): DefaultJson {
            return DefaultJson(
